...
def prepare_diverse_training_data(identity_path, articles_path, output_path=None, format_weights=None):
    ...
    logging.info(f"Starting diverse format dataset preparation, output will be saved to: {output_path}")
        ...
        for line in f:
            if line.strip():
                try:
                    ...
                    logging.warning(f"Error parsing line in identity file: {line[:50]}...")
        ...
        for line in f:
            if line.strip():
                try:
                    ...
                    logging.warning(f"Error parsing line in framework file: {line[:50]}...")
