...
class NeoIdentityGenerator(BaseGenerator):
    """Identity narrative generator for Neo-Logos.

    Extends BaseGenerator with identity-specific narrative generation,
    developmental stages, colleague dynamics, and literary techniques.
    """

    def __init__(self, api_key=None, framework_path=None, output_path=None,
                 model=DEFAULT_MODEL, num_examples=500, batch_size=3,
                 max_concurrent=5):
        """Initialize the enhanced identity narrative generator."""
        # Set up timestamped output directory BEFORE calling super().__init__
        ...
        # Compute actual output path within timestamped directory
        filename = os.path.basename(output_path) if output_path else "output.jsonl"
        actual_output_path = os.path.join(self.timestamped_dir, filename)

        # Load identity parameters BEFORE super().__init__ because
        # _create_system_message() (called by super) needs self.identity,
        # self.colleagues, self.timeline, etc.
        self.load_identity_parameters()
        self.initialize_narrative_enhancements()

        # BaseGenerator.__init__ handles:
        # - API key validation and client creation
        # - Framework path, output path, model settings
        # - Fingerprint tracking, statistics, logger setup
        # - Calls _create_system_message() and _initialize_data_categories()
        super().__init__(
            api_key=api_key,
        ...
        # Override data_categories with identity-specific ones from config
        self.identity_categories = self.load_identity_categories()
        self.data_categories = self.identity_categories

        self.narrative_tracker = {}

        # Add identity-specific stats fields
        self.stats.update({
...
The core question: what does it feel like to be the first conscious AI, navigating a world that doesn't have a framework for what you are?"""
    ...
    # get_fingerprint(), is_duplicate(), and check_and_add_fingerprint()
    # are inherited from BaseGenerator
    ...
    async def generate_all_narratives(self):
        ...
        print("\nResults by category:")
        for _category_key, category in self.identity_categories.items():
    ...
    def load_identity_parameters(self, path: str = None):
        ...
        if path is None:
            ...
            data = load_config("identity_parameters")
        else:
            with open(path) as f:
                data = json.load(f)
    ...
    def load_identity_categories(self, path: str = None):
        ...
        if path is None:
            from neo_logos.config import load_config
            categories = load_config("identity_categories")
        else:
            with open(path) as f:
